...
Cube.prototype = {
	createVertsAndFaces: function() {
		...
		this.faces = [
			{ shade: "dark",   norm: { x:  1, y:  0, z:  0 }, indices: [ 1, 5, 7, 3 ], }, // +x
			{ shade: "light",  norm: { x:  0, y:  1, z:  0 }, indices: [ 2, 3, 7, 6 ], }, // +y
			{ shade: "medium", norm: { x:  0, y:  0, z:  1 }, indices: [ 4, 5, 7, 6 ], }, // +z
			{ shade: "dark",   norm: { x: -1, y:  0, z:  0 }, indices: [ 0, 4, 6, 2 ], }, // -x
			{ shade: "light",  norm: { x:  0, y: -1, z:  0 }, indices: [ 0, 1, 5, 4 ], }, // -y
			{ shade: "medium",  norm: { x:  0, y:  0, z: -1 }, indices: [ 0, 1, 3, 2 ], }, // -z
		];
	},
	computePerspVisibility: function(cam) {
		var angle;
		var i;
		var norm,face;
		var visibles = [];
		for (i=0; i<6; i++) {
			norm = this.faces[i].norm;
			var relcam = {
				x: cam.x - (norm.x + this.pos.x),
				y: cam.y - (norm.y + this.pos.y),
				z: cam.z - (norm.z + this.pos.z),
			};
			var dist = Math.sqrt(relcam.x*relcam.x + relcam.y*relcam.y + relcam.z*relcam.z);
			relcam.x /= dist;
			relcam.y /= dist;
			relcam.z /= dist;
			angle = Math.abs(Math.acos(relcam.x*norm.x + relcam.y*norm.y + relcam.z*norm.z));
			this.faces[i].isVisible = (angle <= Math.PI/2);
		}
	},
	computeOrthoVisibility: function(cam) {
		cam = cam || { x: -1, y: -1, z: 1 };
		var dist = Math.sqrt(cam.x*cam.x + cam.y*cam.y + cam.z*cam.z);
		cam.x /= dist;
		cam.y /= dist;
		cam.z /= dist;
		...
		for (i=0; i<6; i++) {
			norm = this.faces[i].norm;
			angle = Math.abs(Math.acos(cam.x*norm.x + cam.y*norm.y + cam.z*norm.z));
			this.faces[i].isVisible = (angle <= Math.PI/2);
		}
	},
	...
	update: function(dt) {
		this.rotateVerts('y','z', dt*Math.PI/8);
		this.rotateVerts('x','z', dt*Math.PI/3);
		this.rotateVerts('x','y', dt*Math.PI/2);
	},
	draw: function() {
		this.computeOrthoVisibility();
		//this.computePerspVisibility(camera.camPos);
...
Camera.prototype = {
	...
	drawPaper: function() {
		...
		spaceCtx.lineTo(corners[2]);
		spaceCtx.lineTo(corners[3]);
		ctx.closePath();
		ctx.fillStyle = "rgba(255,255,255,0.3)";
		ctx.fill();
	},
	drawCube: function() {
		this.cube.computePerspVisibility(this.camPos);
